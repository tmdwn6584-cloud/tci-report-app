...
export default function ResultContent({ initialResultData }: ResultContentProps) {
  const searchParams = useSearchParams();
  const queryName = searchParams.get("n") || "사용자";
  const queryGender = searchParams.get("g") || "비공개";
  const queryAge = searchParams.get("age") || "";
  const queryMode = searchParams.get("mode") === "lite" ? "lite" : "full";
  ...
  useEffect(() => {
    const computeResults = () => {
      if (initialResultData) {
        const answers = initialResultData.answersString.split("").map(Number);
        const targetQuestions = initialResultData.mode === "lite" ? questions.slice(0, 20) : questions;
        setData(calculateScores(targetQuestions, answers));
        return;
      }
      ...
      if (a) {
        ...
        const targetQuestions = queryMode === "lite" ? questions.slice(0, 20) : questions;
    ...
    computeResults();
  }, [initialResultData, searchParams, queryName, queryGender, queryAge, queryMode]);
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      link.href = dataUrl;
      link.download = `${initialResultData?.name ?? queryName}_전체분석리포트.png`;
      link.click();
  ...
  const mode = initialResultData?.mode ?? queryMode;
  const name = initialResultData?.name ?? queryName;
  const gender = initialResultData?.gender ?? queryGender;
  const age = initialResultData?.age ?? queryAge;
  ...
  return (
    <div className="pb-24 bg-[#fdfbf7] min-h-screen font-sans w-full max-w-[100vw] overflow-x-hidden">
      <div ref={resultRef} className="w-full max-w-full">
        ...
        {queryMode === 'lite' && !initialResultData && (
          <div className="mt-8 md:mt-12 text-center px-4 md:px-0 w-full max-w-4xl mx-auto">
            <Link href={`/test?n=${encodeURIComponent(queryName)}&g=${encodeURIComponent(queryGender)}&age=${encodeURIComponent(queryAge)}&mode=full`} className="inline-flex items-center justify-center mx-auto px-6 py-4 bg-white border border-purple-200 text-purple-700 hover:bg-purple-50 rounded-2xl shadow-sm transition-all text-sm md:text-base font-medium">
            ...
            <div className="z-10 w-full px-12 flex flex-col gap-12 mt-16 flex-1 justify-center">
              <div className="bg-white/80 backdrop-blur-xl p-16 rounded-[4rem] shadow-2xl border border-white">
                ...
                <p className="text-[2.5rem] text-gray-600 leading-normal font-light break-keep">
                  "당신은 관계 자체보다, 관계의 감정 흐름 변화에 더 크게 반응하는 경향이 있습니다. 
                  가까워질수록 상대 반응과 거리감 변화를 더 세밀하게 읽어낼 가능성이 있습니다."
                </p>
              </div>
